...
3. Prefer `scripts/git/get-ci.sh` when available, then inspect the current CI state, failed job names, pending jobs, canceled jobs, skipped jobs, and relevant logs. Use `scripts/git/gh/get-ci.sh` or `scripts/git/glab/get-ci.sh` directly only when the provider target is already explicit.
