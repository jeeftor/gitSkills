...
Read `references/git-workflow/common.md`, `references/git-workflow/helpers.md`, and `references/git-workflow/table.md`. For the normal current-checkout path, call `scripts/git/get-issues.sh --state open --scope all --limit 50`; it resolves the current GitHub or GitLab repository and delegates to the provider helper. Read `references/git-workflow/target-resolution.md` only when the user names a URL, named remote, issue scope, or multi-remote target that the helper cannot resolve directly.

## Workflow

1. Identify the issue scope: assigned to the user, authored by the user, or all open issues. Treat bare `all` as all open issues for the resolved repository, not all remotes.
2. Use `scripts/git/get-issues.sh --state open --scope <all|authored|assigned> --limit 50` for the default repository target.
3. For a named remote such as `origin` or `upstream`, pass it to the helper as the target: `scripts/git/get-issues.sh <remote> --state open --scope <all|authored|assigned> --limit 50`.
...
- `$git-issue-table authored` - list open issues authored by the current platform user.
- `$git-issue-table assigned` - list open issues assigned to the current platform user.
